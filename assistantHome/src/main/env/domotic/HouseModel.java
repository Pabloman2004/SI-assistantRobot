...
public class HouseModel extends GridWorldModel {
	...
	//es un hashmap con el nombre del medicamento y la cantidad disponible
	...
	*/
    ...
    public HouseModel() {
		...
		addWall(GSize+2, GSize/2, GSize*2-1, GSize/2);  
		 ...
		 medicamentos.put("Simvastatina", 5);
		 medicamentos.put("Aspirina ", 5);
		 medicamentos.put("Omeprazol ", 5);
		 medicamentos.put("Lexotiroxina sódica", 5);
    ...
    boolean getDrug(String medicamento) {
        if (fridgeOpen && medicamentosDisponible(medicamento) > 0 && !carryingDrug) {
			...
			if ( medicamentosDisponible(medicamento)>0){ 
    ...
    boolean addDrug(int n) {
        for (String medicamento : medicamentos.keySet()){
			if (medicamentos.get(medicamento) == 0){
				medicamentos.put(medicamento, n);
			}
		}
        //if (view != null)
    ...
    boolean handInDrug() {
        if (carryingDrug) {
            sipCount = 10;//esto es como el tiempo en el que tarda en tomar el medicamento
            carryingDrug = false;
            //if (view != null)
	...
	int medicamentosDisponible(String medicamento){
		
		return medicamentos.get(medicamento);
